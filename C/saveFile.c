...
char** temp=tab;
char* ligne=(char*)malloc((TAILLE_MAX)*sizeof(char)); //Contiendra le contenu d'une ligne à la fois.
...
while(fgets(ligne,TAILLE_MAX,fichier) != NULL){
	temp[j] = (char*)malloc((TAILLE_MAX)*sizeof(char));
	strcpy(temp[j],ligne);
	printf("ligne %d : taille : %d\n : %s",j,strlen(ligne),temp[j]);
	j++;
}
printf("%d\n",j);

//while(fgets(ligne,TAILLE_MAX,fichier) != NULL) {
//	temp[j] = (char*)malloc((strlen(ligne))*sizeof(char));
//	printf("ligne %d : taille : %d\n",j,strlen(ligne));
	//strcpy(temp[j],ligne);
//	j++;
//}
